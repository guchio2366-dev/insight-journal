...
test('確認ページでも分野をURLから復元する',()=>{
 assert.equal(readAtlasState(new URL('https://example.org/insight-journal/atlas/north-america/review/?field=land')).field,'natural');
...
test('地図の分野と位置・倍率をURLから復元する',()=>{
 ...
 assert.equal(state.field,'natural');assert.deepEqual(state.camera,{lng:-100,lat:40,zoom:4.125});
...
test('分野の切替で位置・倍率を維持し、旧タップ領域のURLを除去する',()=>{
 const next=writeAtlasState(new URL('https://example.org/insight-journal/atlas/north-america/agriculture/?zone=zone-corn-belt&insight=old'),'/insight-journal/atlas/north-america/','natural',{lng:-92.1,lat:41.2,zoom:5.23},'corn','midwest-corn-soy','corn','fit');
 assert.equal(next.pathname,'/insight-journal/atlas/north-america/nature/');assert.equal(next.searchParams.get('z'),'5.2300');assert.equal(next.searchParams.has('zone'),false);
