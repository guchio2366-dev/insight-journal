...
export function renderPopulationComposition(host:HTMLElement,items:PopulationSlice[],label:string,options:{remainderLabel?:string}={}){
 ...
 const listed=[...items];if(model&&model.remainder>0.000001)listed.push({label:options.remainderLabel??'未表示分',value:model.remainder,color:'#c8ccd0'});
