...
export async function startAtlas() {
  ...
  function save(push=false){
    if(restoring)return;
    const url=industries.write(writeAtlasState(new URL(location.href),config.base,field,cameraState(),selectedCrop,selectedRegion,selectedStats,view,[...agriLayers] as any,selectedAnimal,selectedAnimalRegion,{env:natureMode,city:selectedCity,natureFeature}));
    ...
    root.querySelectorAll<HTMLAnchorElement>('[data-cross-link="agriculture"]').forEach(link=>{
      const target=writeAtlasState(new URL(url),config.base,'agriculture',cameraState(),selectedCrop,selectedRegion,selectedStats,view,[...agriLayers] as any,selectedAnimal,selectedAnimalRegion,{env:natureMode,city:selectedCity,natureFeature});
  ...
  function placeSelectionCard(){
    ...
    for(const node of root.querySelectorAll<HTMLElement>('.atlas-map-tools,[data-layer-caption],.atlas-geolabel:not([hidden]),.industry-marker'))protectedRects.push(localRect(node));
  ...
  function hideSelection(){
    ...
    currentCopy=null;selectedCoordinate=null;natureSelectedGeometry=null;
  ...
  function closeSelection(saveUrl=true){
    if(field==='agriculture'){selectedAnimal=null;selectedAnimalRegion=null;selectedCrop=null;selectedRegion=null;root.querySelectorAll('[data-crop-select]').forEach(node=>node.removeAttribute('aria-current'));}
    ...
    hideSelection();root.querySelectorAll<HTMLSelectElement>('[data-feature-select]').forEach(select=>select.value='');updateFocusButton();if(saveUrl)save();
  }
  ...
  function selectCity(id:string,saveUrl=true){
    ...
    if(saveUrl)save(true);
  }

  function showNatureFeature(key:string,title?:string,coordinate?:[number,number],saveUrl=true){
    if(!validNatureFeature(key))return;
  ...
  function fallbackForCurrentView(){
    ...
    if(field==='natural')return {base:config.natureAssetBase,name:`${natureMode}-fallback.webp`,alt:'米国本土の自然環境'};
  ...
  async function ensureNatureModeData(mode:NatureMode){
    ...
    try{
      let data:any=null;
      if(mode==='climate'&&!climatePixels){
        const response=await fetch(config.natureAssetBase+'climate-classes.png');if(!response.ok)throw Error('Climate classes');
  ...
  function restoreSelectionForView(){
    hideSelection();
    if(field==='agriculture'&&selectedAnimalRegion)selectLivestock(selectedAnimalRegion,false);
  ...
  function setNatureMode(next:NatureMode,push=false){
    ...
    natureMode=next;root.dataset.natureMode=next;if(field==='natural')el('[data-map-panel]').setAttribute('aria-labelledby',`nature-tab-${next}`);
  ...
  function fail(message:string){
    if(failed)return;failed=true;ready=false;clearTimeout(timeout);criticalController.abort();root.dataset.renderState='fallback';fallback.hidden=false;surface.hidden=true;el('[data-map-labels]').hidden=true;el('[data-livestock-markers]').hidden=true;el('.atlas-map-tools').hidden=true;status.textContent=message+'（代替図）';map?.remove();map=undefined;industries.renderMarkers();if(!selection.hidden)moveSelectionBelow();
  }
  ...
  try{
    ...
    map.once('load',()=>{
      ...
      const cityLabels=config.climateCities.map((city:ClimateCity,index:number)=>({id:'city-'+city.id,name:city.nameJa,lng:city.longitude,lat:city.latitude,kind:'city',priority:index<6?2:3,fields:['natural'],modes:['climate']}));
      ...
      map.on('move',()=>{renderLabels();renderLivestockMarkers();industries.renderMarkers();if(!selection.hidden&&selection.parentElement===frame)selection.style.visibility='hidden';});map.on('moveend',()=>{if(!suppressNextMove)view='custom';else suppressNextMove=false;renderLabels();renderLivestockMarkers();industries.renderMarkers();placeSelectionCard();updateFocusButton();save();});
      map.on('click',event=>{
        ...
        if(field==='natural'){
          ...
          if(natureMode==='climate'){const city=map!.queryRenderedFeatures(box,{layers:['city-points']})[0];if(city)selectCity(city.properties.id);else if(climatePixels){const index=climateCell(event.lngLat.lng,event.lngLat.lat,climatePixels.width,climatePixels.height,climateBounds);const id=index===null?0:climatePixels.data[index];const item=config.climateLegend.find((item:any)=>item.id===id);if(item)showNatureFeature('climate:'+item.code,undefined,[event.lngLat.lng,event.lngLat.lat]);else closeSelection();}else closeSelection();return;}
