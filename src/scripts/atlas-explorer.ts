...
export async function startAtlas() {
  ...
  root.addEventListener('click',event=>{
    ...
    const heading=el<HTMLElement>(root.dataset.productStory?'#agri-insight-heading':field==='agriculture'?'#agri-reading-heading':link.hasAttribute('data-forest-compare')?'#forest-comparison-heading':link.hasAttribute('data-nature-reading-link')?(water.active()?'#water-reading-title':natureFeature?'#nature-feature-heading':natureMode==='climate'?'#city-climate-heading':'#nature-map-panel'):'#agri-insight-heading');
    if(heading){heading.tabIndex=-1;heading.focus({preventScroll:true});}
    if(link.matches('[data-agri-insight-link],.agri-insight-back')){
      const body=el('[data-agri-reading-body]');body.scrollTop=0;
      const panel=root.dataset.productStory?el('.agri-insight-context'):el('[data-agri-reading-panel]');
