...
export function init(root:HTMLElement){
 ...
 const read=()=>readLatinState(location.search,topics,config.countries.map((c:any)=>c.code),cities);
 ...
 function commit(next:LatinState,options:{replace?:boolean;keepView?:boolean;comparison?:boolean}={}){
  ...
  const url=new URL(location.href);url.search=writeLatinState(state);
 ...
 function render(move=true){
  ...
  all<HTMLButtonElement>('[data-field]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.field===state.field)));
  ...
  q<HTMLSelectElement>('[data-city]').value=state.city;q<HTMLSelectElement>('[data-view]').value=state.view;q<HTMLSelectElement>('[data-crop]').value=state.crop;
  q('[data-nature-controls]').hidden=state.field!=='nature';q('[data-agriculture-controls]').hidden=state.field!=='agriculture';
  q('[data-map-title]').textContent=state.field==='nature'?(state.view==='climate'?'気候と地域の違い':'河川・地形を読む'):state.field==='agriculture'?(isLivestock?'家畜の分布':state.crop==='none'?'農林業の地域を読む':'作物が育つ場所'):state.field==='industry'?'資源・加工・物流の拠点':'都市と人口分布の背景';
  q('[data-map-kicker]').textContent=state.field==='nature'?(state.view==='climate'?'Climate · 1991–2020':'Rivers · Natural Earth'):state.field==='agriculture'?(isLivestock?'Livestock density · 2020':state.crop==='none'?'Agriculture · Regional reading':'Harvested area · 2020'):state.field==='industry'?'Industry · Regional reading':'Population density · 2020';
  q('[data-overview]').hidden=Boolean(state.topic||state.city);q('[data-overview-title]').textContent=field.subtitle;q('[data-overview-summary]').textContent=field.summary;q('[data-reading-field]').textContent=field.name;
  all('[data-topic-panel]').forEach(e=>e.hidden=e.dataset.topicPanel!==state.topic);
  all('[data-city-panel]').forEach(e=>e.hidden=e.dataset.cityPanel!==state.city);
  ...
  empty.hidden=visible.size>0;empty.textContent='この国に対応する個別解説はありません。地図の分布と下の国別統計を参照できます。「中南米全体」で周辺地域の解説を読めます。';
  ...
  if(move)fit();
  drawMarkers();loadRaster();
 }
 function markerRows(){
  const rows=visibleTopics().map(t=>({id:t.id,label:t.label,lon:t.location[0],lat:t.location[1],num:topics.indexOf(t)+1,city:false}));
  if(state.field==='nature')for(const c of cities.filter(c=>!state.place||c.countryCode===state.place))rows.push({id:c.id,label:c.name,lon:c.longitude,lat:c.latitude,num:0,city:true});
  return rows;
 }
 ...
 function drawMarkers(){
  ...
  if(mapReady){
   for(const r of rows){
    const b=document.createElement('button');b.type='button';b.className=`latin-marker${r.city?' climate-city':''}`;b.setAttribute('aria-label',`${r.city?'雨温図':'解説'}：${r.label}`);b.setAttribute('aria-pressed',String(r.id===(r.city?state.city:state.topic)));b.textContent=r.city?'○':String(r.num);b.title=r.label;
    const label=document.createElement('span');label.textContent=r.label;b.append(label);
    ...
    markers.push(new (window as any).__latinMarker({element:b,anchor:'center'}).setLngLat([r.lon,r.lat]).addTo(map));
   }
  }else{
   ...
   for(const r of rows){
    ...
    const circle=document.createElementNS(g.namespaceURI,'circle');circle.setAttribute('r',String((r.city?6:12)*scale));circle.setAttribute('fill',r.city?'#fff':r.id===state.topic?'#a34532':'#183d51');circle.setAttribute('stroke','#fff');circle.setAttribute('stroke-width',String(2*scale));g.append(circle);
    if(!r.city){const text=document.createElementNS(g.namespaceURI,'text');text.textContent=String(r.num);text.setAttribute('font-size',String(12*scale));text.setAttribute('text-anchor','middle');text.setAttribute('dominant-baseline','central');text.setAttribute('fill','#fff');g.append(text);}
 ...
 function legend(items:{color:string;label:string}[],note:string){
  ...
  const p=document.createElement('p');p.textContent=note;el.append(list,p);
 }
 async function loadRaster(){
  ...
  q('[data-grid-reading]').textContent='地図の番号かテーマ一覧から、地域の解説を選べます。';
  try{
   ...
   }else{if(ticket!==rasterRequest)return;legend(field==='nature'?[{color:'#4f91b2',label:'主な河川'}]:[],'番号は解説する地域・拠点の代表位置です。面積や生産量の大小を表しません。');}
 ...
 root.addEventListener('click',e=>{
  ...
  else if(b.hasAttribute('data-fit')){state.camera=undefined;fit();drawMarkers();}
 ...
 q<HTMLSelectElement>('[data-crop]').addEventListener('change',e=>commit({...state,crop:(e.target as HTMLSelectElement).value},{keepView:true}));
 ...
 window.addEventListener('resize',()=>{if(!mapReady)drawMarkers()});
 render();
 ...
 Promise.all([json(base+'latin-america-context-v1/countries.json'),json(base+'latin-america-context-v1/rivers.json')]).then(async([countries,rivers])=>{
  ...
  try{
   map=new Map({container:q('[data-map-surface]'),attributionControl:false,cooperativeGestures:true,center:[-62,-15],zoom:2,minZoom:1,maxZoom:9,maxBounds:[[-105,-65],[-20,40]],style:{version:8,sources:{countries:{type:'geojson',data:countries},rivers:{type:'geojson',data:rivers}},layers:[{id:'sea',type:'background',paint:{'background-color':'#e7eff3'}},{id:'countries-fill',type:'fill',source:'countries',paint:{'fill-color':['case',['in',['get','code'],['literal',config.countries.map((c:any)=>c.code)]],'#d8e0da','#dddfe0']}},{id:'countries-line',type:'line',source:'countries',paint:{'line-color':'#6b7c80','line-width':.8}},{id:'rivers-line',type:'line',source:'rivers',paint:{'line-color':'#4f91b2','line-width':1.4,'line-opacity':.8}}]}});
   ...
   map.on('moveend',()=>{if(!mapReady)return;state.camera=currentCamera();const url=new URL(location.href);url.search=writeLatinState(state);history.replaceState({latinReturn:returnTo},'',url)});
