import {populationViews,ethnicities,religions,religionNational,metros,populationClasses,densityColors,shareColors,voteColors,missingColor} from '../data/atlas/population';
...
export function createPopulationController(root:HTMLElement,base:string,options:{active:()=>boolean;map:()=>any;changed:(push?:boolean)=>void;fit:(bounds:any)=>void}){
 ...
 function selected(){
  ...
  if(state.view==='religion'){text=`${religions.find(x=>x[0]===state.religion)?.[1]}：州別データ未取得`;note='成人の調査。全国値から州の値を推定しません。';}
 ...
 function national(){
  ...
  if(state.view==='religion')bars(religions.map((x,i)=>[x[1],religionNational[i]]),'成人の自己認識','Pew 2023–24年調査。人口統計とは分母が異なります。');
  ...
  el('[data-pop-national-footnote]').textContent=state.view==='distribution'?'郡分類の人口加重集計です。中心市街地・郊外の居住人口を直接分けた比率ではありません。':state.view==='religion'?'未取得の2区分を0と見なさないでください。丸めのある分類を合算して補いません。':'全国は50州とDC。地図は米国本土。全国の棒グラフは表示切替ボタンではありません。';
 }
