...
export function industryStateComparison(sector:string,subsector:string){
 ...
 const rows=field.rows.map(r=>({...data.states.find(s=>s.id===r.id)!,...r}));
 ...
 return {...field,sourceInfo:data.sources[field.source as keyof typeof data.sources],max,total:points.length,rows,points};
}
